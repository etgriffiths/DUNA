function hdf5write_ICES(filename, varargin)
%HDF5WRITE  Write a Hierarchical Data Format version 5 file.
%    This is a direct copy of the old hdf5write function originally created
%    by Matlab and works just fine for ICES data formatting. 
%
...
%
%      Location     The location of the dataset in the file specified as a
%                   character vector or string scalar
%
%      Name         A name to attach to the dataset specified as a
%                   character vector or string scalar
%
...
%
%      AttachType   Defines the kind of object this attribute modifies.
%                   Possible values are 'group' and 'dataset'.
%
%      Name         A name to attach to the dataset specified as a
%                   character vector or string scalar
%
...
if nargin > 1
    ...
    varargin = convertStringsInDetails(varargin);
    validateCellArray(varargin);
end
...
function out = convertStringsInDetails(in)
% Function to convert string fields of the details struct into char
% vectors. This performs conversion only if value is a string and leaves it
% unchanged otherwise. It does not recursively probe the value.
...
for cnt = 1:numel(in)
    ...
    % The conversion needs to be applied to every field in every element of
    % the struct array. STRUCTFUN does not operate directly on arrays.
    if numel(in{cnt}) > 1
        sfHandle = @(inArg) structfun(@(x) convertStringsToChars(x), inArg, 'UniformOutput', false);
        out{cnt} = arrayfun( @(x) sfHandle(x), in{cnt});
    else
        out{cnt} = structfun(@(x) convertStringsToChars(x), in{cnt}, 'UniformOutput', false);
    end
end

function validateCellArray(in)
% Function that checks that cell arrays do not contain strings OR a
% combination of char vectors and strings. This is not a paradigm that we
% want to recommend to our users.

for cnt = 1:numel(in)
    % No action needs to be taken if the input argument is a cellstr or not
    % a cell array at all.
    if ~iscell(in{cnt}) || iscellstr(in{cnt})
        continue;
    end
    
    matlab.io.internal.imagesci.validateTextInCell(in{cnt}, 'deprecatedHDF5');
end
