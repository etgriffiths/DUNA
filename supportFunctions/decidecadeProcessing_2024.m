...
%  This is Step 1.
%
% Current Version by Emily T. Griffiths, 2020-2024, modified as a function
% to work within DUNA.
%
% Version (c) 2024
%
...
function [outputLocation, results, resultname] = decidecadeProcessing(depData)
    ...
    %File name
    resultname=[depData.Deployment_ID '_to_' datestr(depData.DTslut, 'yyyymmdd') '_' depData.loggerID  ]; %save results in mat file with this name
    ...
    for fileno = 1:height(filesdata)
        ...
        [sig,sr] = audioread([depData.dataPath '\' char(filesdata.name(fileno))]) ;      % Reads the file, output: signal (sig) and sample rate (sr).
